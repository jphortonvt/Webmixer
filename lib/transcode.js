...
async function renderMixdown(cacheDir, sessionId, trackFiles, settings, options = {}) {
  const { format = 'mp3', startSeconds, endSeconds } = options;
  const sessionCacheDir = path.join(cacheDir, sessionId);
  ...
  await new Promise((resolve, reject) => {
    execFile('ffmpeg', args, { maxBuffer: 1024 * 1024 * 8 }, (err, stdout, stderr) => {
      if (err) reject(new Error(`ffmpeg mixdown failed: ${err.message}\n${stderr}`));
      else resolve();
    });
  });
