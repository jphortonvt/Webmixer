...
async function transcodeSession(cacheDir, sessionId, trackFiles) {
  ...
  // Default to 2 concurrent transcodes to avoid resource/memory exhaustion (OOM) on restricted server tiers.
  const CONCURRENCY_LIMIT = parseInt(process.env.TRANSCODE_CONCURRENCY || '2', 10);
  console.log(`[TRANSCODE] Starting session ${sessionId} with concurrency limit of ${CONCURRENCY_LIMIT}...`);
