const { S3Client, ListObjectsV2Command, GetObjectCommand, PutObjectCommand, HeadObjectCommand, DeleteObjectCommand, PutBucketCorsCommand } = require('@aws-sdk/client-s3');
...
const TRACK_PATTERN = /^TRACK\d+([LR]|_ST)?\.WAV$/i;
const STEREO_PAIR_PATTERN = /^TRACK(\d+)([LR])\.WAV$/i;
const STEREO_ST_PATTERN = /^TRACK\d+_ST\.WAV$/i;
// Zoom project metadata and Ableton clip-analysis sidecars — expected, not tracks
const KNOWN_SIDECAR_PATTERN = /(^L8PRJDATA\.ZDT$|\.asd$)/i;
...
async function listSessionTracks(sessionId) {
  ...
  const allNames = allContents.map(obj => path.basename(obj.Key));
  const matched = allNames.filter(name => TRACK_PATTERN.test(name)).sort();

  // Warn only about files we don't recognise — a new recorder naming scheme
  // would otherwise be silently dropped, as TRACK*_ST.WAV once was. Known
  // sidecars (Zoom project data, Ableton analysis files) are expected.
  const unexpected = allNames.filter(name =>
    !TRACK_PATTERN.test(name) && !KNOWN_SIDECAR_PATTERN.test(name)
  );
  if (unexpected.length > 0) {
    console.warn(`[b2] ${sessionId}: unrecognised file(s) not loaded as tracks: ${unexpected.join(', ')}`);
  }

  // Separate L/R pair candidates from plain track files
  const pairSides = new Map(); // track number string -> { L, R }
  const singles = [];

  for (const name of matched) {
    const m = name.match(STEREO_PAIR_PATTERN);
    if (m) {
      const num = m[1];
      if (!pairSides.has(num)) pairSides.set(num, {});
      pairSides.get(num)[m[2].toUpperCase()] = name;
    } else if (STEREO_ST_PATTERN.test(name)) {
      singles.push({ name, type: 'stereo' });
    } else {
      singles.push({ name, type: 'mono' });
    }
  }

  const tracks = [...singles];

  for (const [num, sides] of pairSides) {
    if (sides.L && sides.R) {
      const merged = `TRACK${num}.WAV`;
      console.log(`[b2] ${sessionId}: stereo pair → ${merged} (${sides.L} + ${sides.R})`);
      tracks.push({ name: merged, type: 'stereo_pair', left: sides.L, right: sides.R });
    } else {
      // Only one side present — treat as a plain mono track
      const orphan = sides.L || sides.R;
      console.log(`[b2] ${sessionId}: unpaired L/R file, treating as mono: ${orphan}`);
      tracks.push({ name: orphan, type: 'mono' });
    }
  }
...
/**
 * Delete an object from B2. The bucket keeps versions, so this leaves a
 * delete marker rather than destroying the file outright.
 */
